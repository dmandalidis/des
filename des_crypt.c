...
void create_keys(unsigned long long key, unsigned long long *skey)
{
	...
	unsigned char erot[] = { 1, 1, 2, 2, 2, 2, 2, 2, 1, 2, 2, 2, 2, 2, 2, 1};
...
void encrypt_block(volatile unsigned char *msg, volatile unsigned char *imsg, unsigned long long *skey)
{
	...
	for (i = 0; i < 16; i++) {
        n_upper = lower;
        n_lower = upper ^ f((unsigned int) lower, skey[i]);
        lower = n_lower;
        upper = n_upper;
    }
